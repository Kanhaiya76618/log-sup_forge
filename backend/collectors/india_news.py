...
logger = logging.getLogger("flowforge.collectors.india_news")
...
class IndiaNewsCollector:
    """Collects India-specific maritime and port disruption news using NewsAPI."""
    ...
    def fetch_news(self) -> List[Dict[str, Any]]:
        if not self._has_valid_key():
            logger.warning("NEWS_API_KEY not set — returning empty India news feed.")
            return []
        
        # Combine specific search queries targeting India shipping delays/disruptions
        query = "India AND (port OR ports OR shipping OR logistics) AND (disruption OR delay OR strike OR JNPT OR Mundra OR Chennai)"
        ...
        try:
            ...
            logger.info(f"Fetched {len(articles)} India news articles from NewsAPI.")
            return [_normalize_article(a) for a in articles if a.get("title")]
        except requests.Timeout:
            raise RuntimeError("NewsAPI India request timed out")
        except requests.HTTPError as e:
            raise RuntimeError(f"NewsAPI India returned HTTP error: {e}")
        except requests.RequestException as e:
            raise RuntimeError(f"NewsAPI India request failed: {e}")
        return []

india_news_collector = IndiaNewsCollector()

def get_india_news() -> List[Dict[str, Any]]:
    return india_news_collector.fetch_news()
